...
# Helper to replace method stubs cleanly without caring about line endings
def inject_method(method_name, impl_body):
    global content
    pattern = r'    private static int ' + method_name + r'\(CommandContext<CommandSourceStack> context\) \{[\s\S]*?return 1;\s*\}'
...
meth_antioch = '''    private static int executeAntioch(CommandContext<CommandSourceStack> context) throws CommandSyntaxException { return executeAntioch(context, null); }
...
# Only if not already intercepted
if 'saveBackLocation(player);' not in content:
...
meth_back = '''    private static int executeBack(CommandContext<CommandSourceStack> context) throws CommandSyntaxException { return executeBack(context, java.util.Collections.singletonList(context.getSource().getPlayerOrException())); }
...
meth_ice = '''    private static int executeIce(CommandContext<CommandSourceStack> context) throws CommandSyntaxException { return executeIce(context, java.util.Collections.singletonList(context.getSource().getPlayerOrException())); }
